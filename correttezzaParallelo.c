/*
Progetto: Forrest Fire
Studente: Gaetano Di Genio
Mat ultime 5 cifre: 01339
*/
...
#define E "💨"
#define B "🔥"
#define S 10 //numero iterazioni totali
#define P 0  //albero cresce in cella vuota con probabilità P
#define F 0 //albero brucia con probabilità F se nessun vicino sta bruciando
...
int main(int argc, char* argv[]){
    ...
    int vuota = 0; //conteggio caselle vuote tutti i processi
    int ok = 0; //segnale da p0 a tutti
    int emptCtr; //p0
    int emptMaster;
    ...
    */
    ...
    for(int i = 0; i<S; i++){
        ...
        MPI_Gatherv(SUBMAT, sendcounts[me], MPI_CHAR, MAT2, sendcounts, offset, MPI_CHAR, 0, MPI_COMM_WORLD);

        if(me == 0){
            if(m <= 10 && n <= 10){
                printf("Matrice iterazione %d: \n", i);
